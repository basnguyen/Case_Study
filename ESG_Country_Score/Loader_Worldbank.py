...
for key_i in indicators_tofetch:
    ...
    try:
        ...
        WBDataframe = wbdata.get_dataframe({key_i: id_i}, convert_date=False).rename(
            index=ISO_mapping.set_index("Country Name").to_dict()["Country Code"]
        )

        # Check that all country names are correctly mapped to an ISO code, loader will fail if it is not empty
        list_of_masters = pd.Series(
            WBDataframe.index.get_level_values(0),
        ).drop_duplicates(keep="first")
        list_non_ISO = list_of_masters[
            (list_of_masters.apply(lambda x: len(x) > 3))
            | (list_of_masters.str.contains(r"[a-z]"))
        ]
    ...
    try:
        ...
        con = sqlite3.connect("../SQL_Database/ESG_Database_DEV.sqlite3")
        cur = con.cursor()
        ...
        cur.execute(
            ...
            (
                SELECT country, max(date) FROM {id_i} 
                WHERE value IS NOT NULL GROUP BY country
            )
            """
        )
        ...
        cur.execute(f"""
            ...
            SELECT country, date, 100*(value - min_value) / (max_value -min_value) normalized_value
            FROM {id_i}_LATEST  
            CROSS JOIN (
